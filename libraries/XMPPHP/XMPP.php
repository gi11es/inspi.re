...
class XMPPHP_XMPP extends XMPPHP_XMLStream {
	...
	public function publish($to, $body, $node) {
		$to	  = htmlspecialchars($to);
		$body = htmlspecialchars($body);
		$node = htmlspecialchars($node);
		
		$out = "<iq type=\"set\" to=\"$to\" id=\"publish1\">";
		$out .= "<pubsub xmlns=\"http://jabber.org/protocol/pubsub\">";
		$out .= "<publish node=\"$node\">";
		$out .= "<item id=\"lastupdate\">";
		$out .= "<entry xmlns='http://www.w3.org/2005/Atom'>";
		$out .= "<body>$body</body>";
		$out .= "</entry>";
		$out .= "</item>";
		$out .= "</publish>";
	...
	public function register($username, $password) {
		...
		$out .= "<username>$username</username><password>$password</password>";
		$out .= "</query></iq>";
