...
class XMPPHP_XMLStream {
	...
	public function startXML($parser, $name, $attr) {
		...
		if(array_key_exists('XMLNS', $attr)) {
			...
			if (isset($this->current_ns[$this->xml_depth - 1])) $this->current_ns[$this->xml_depth] = $this->current_ns[$this->xml_depth - 1];
	...
	public function endXML($parser, $name) {
		...
		if($this->xml_depth == 1) {
			...
			foreach($this->idhandlers as $id => $handler) {
				if(isset($this->xmlobj[2]) && array_key_exists('id', $this->xmlobj[2]->attrs) and $this->xmlobj[2]->attrs['id'] == $id) {
