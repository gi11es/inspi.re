...
$discussionpostlist = DiscussionPostList::getByIndexingStatusAndStatus($INDEXING_STATUS['INDEXED'], $DISCUSSION_POST_STATUS['POSTED']);
$processcount = 0;
...
foreach ($discussionpostlist as $oid => $creation_time) {
	try {
		...
		if ($discussionthread->getEid() === null) {
			...
			$discussionpost->setIndexingStatus($INDEXING_STATUS['UNINDEXED']);
...
$discussionthreadlist = DiscussionThreadList::getByIndexingStatusAndStatus($INDEXING_STATUS['INDEXED'], $DISCUSSION_THREAD_STATUS['ACTIVE']);
...
foreach ($discussionthreadlist as $nid => $creation_time) {
	try {
		...
		$discussionthread->setIndexingStatus($INDEXING_STATUS['UNINDEXED']);
		$processcount++;
	} catch (DiscussionThreadException $e) {}
}
