...
/* 
       ...
       Adds an entry to a user's list of favorites
*/
...
if ($entry !== null) {
	...
	if (!in_array($entry->getEid(), array_keys($favoritelist))) {
		$favorite = new Favorite($entry->getEid(), $user->getUid());
		Log::xmpp('GENERAL_ACTIVITY', '<profile_picture uid="'.$user->getUid().'" size="tiny"/><div class="real_time_update_text"><translate id="JABBER_ADD_TO_FAVORITES"><user_name uid="'.$user->getUid().'"/> added an entry to his/her favorites</translate></div>');
	}
}
