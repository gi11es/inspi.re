...
function previousEntry($entrylist, $eid) {
	if (count($entrylist) == 1)
		return Entry::get($eid);
		
	$eids = array_values($entrylist);
	...
	while($current_eid = current($eids)) {
		if ($grabnext) {
			$target_eid = $current_eid;
			break;
		}
		if ($current_eid == $eid)
			$grabnext = true;
	...
	if ($target_eid === null) $target_eid = array_pop($eids);
	
	try {
		$entry = Entry::get($target_eid);
		return $entry;
	} catch (EntryException $e) {
		return previousEntry($entrylist, $target_eid);
	}
}

function previousSortedEntry($sortedentries, $entries, $eid) {
	if (count($sortedentries) == 1)
		return $entries[$eid];
		
	$eids = array_keys($sortedentries);
	...
	if (isset($entries[$target_eid])) return $entries[$target_eid];
	else return previousSortedEntry($sortedentries, $entries, $target_eid);
}
...
if ($entry !== null) {
	...
	if ($uid == $community->getUid() || $ismoderator) {
		...
		asort($sortedentries);
		$entry = previousSortedEntry($sortedentries, $entries, $eid);
		...
		asort($sortedentries);
		$entry = previousSortedEntry($sortedentries, $entries, $eid);
	} else $entry = previousEntry($entrylist, $eid);
